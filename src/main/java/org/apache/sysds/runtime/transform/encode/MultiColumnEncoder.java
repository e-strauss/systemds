	...
	private void applyMT(CacheBlock<?> in, MatrixBlock out, int outputCol, int k, ArrayList<int[]> nnzOffsets) {
		...
		try {
			if(APPLY_ENCODER_SEPARATE_STAGES) {
				...
				int[] currentNnzOffsets = null;
				for (ColumnEncoderComposite e : _columnEncoders) {
					// for now bag of words
					// is only used in encode
...
//	}
			...
			if (mcsr) {
				...
				if(encm.numBOWEnc <= 0 )
					...
					if( encm.nnzPerRowBOW != null) {
						...
						int num_blks1= Math.min( (k + encm.numBOWEnc - 1)/ encm.numBOWEnc, Math.max(num_blocks, 1)); // 1 <= num_blks1 <= k / #enc
						int blk_len1 = (input.getNumRows() + num_blks1 - 1) / num_blks1;
						int num_blks2= Math.min(k, Math.max(num_blocks, 1)); // 1 <= num_blks2 <= k
	...
	private static EncoderMeta getEncMeta(List<ColumnEncoderComposite> encoders, boolean noBuild, int k, CacheBlock<?> in) {
		...
		if(!bows.isEmpty()){
			...
			// Concurrent (column-wise) bag of words nnz estimation per row
			// The sampling might be problematic since we used for the sparsity estimation -> which impacts performance
